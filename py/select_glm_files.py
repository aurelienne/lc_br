...
def write_csv():
    ...
    with open('/home/ajorge/data/grid_percents.csv', 'w', newline='') as csvfile:
...
def write_filtered_csv(df):
    with open('/home/ajorge/data/glm_filtered.csv', 'w', newline='') as csvfile:
...
def create_dataframe(min_percent=0, max_percent=100, min_flashes=0):
    df = pd.read_csv('/home/ajorge/data/grid_percents.csv')
    ...
    plt.savefig('/home/ajorge/figs/percent_nonzero.png')

    df['min_flashes_patch'].hist(grid=False, rwidth=0.9)
    plt.title('Minimum number of flashes per patch')
    plt.ylabel('Absolute Frequency')
    plt.xlabel('Number of flashes')
    #plt.xscale('log')
    plt.savefig('/home/ajorge/figs/min_flashes_patch.png')
...
if __name__ == "__main__":
    #write_csv()
    create_dataframe(min_percent=1.5, max_percent=10, min_flashes=3)
