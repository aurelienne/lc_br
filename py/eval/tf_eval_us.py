...
def parse_tfrecord_control(example):
    ...
    ch05 = tf.reshape(tf.io.parse_tensor(features["CH05"], tf.uint8), [960, 960, 1])
    ...
    ch13 = tf.reshape(tf.io.parse_tensor(features["CH13"], tf.uint8), [480, 480, 1])
    # Unscaling the TensorFlow tensor to native units. Then applying mean/std scaling, since the model was trained thusly.
    ...
    ch15 = tf.reshape(tf.io.parse_tensor(features["CH15"], tf.uint8), [480, 480, 1])
    # Unscaling the TensorFlow tensor to native units. Then applying mean/std scaling, since the model was trained thusly.
    ...
    fed_accum = tf.reshape(
        tf.io.parse_tensor(features["FED_accum_60min_2km"], tf.uint8), [480, 480]
    )  # no last dim here
