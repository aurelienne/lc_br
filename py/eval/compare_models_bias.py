...
def compare_models(pkl_dict1, pkl_dict2):
    ...
    for idx, preds1 in enumerate(pkl_dict1['preds']):
        ...
        # Calculate differences only where pixels represent at least 1% of probability
        mask = (preds1 >= 0.01) & (preds2 >= 0.01)
        difs = np.where(mask, preds1 - preds2, np.nan)
        filtered_values = difs[~np.isnan(difs)]
        for val in filtered_values.tolist():
            difs_list.append(val)

    avg_dif = np.mean(difs_list)
    print(f"Avg Dif = {avg_dif}")
    return avg_dif
...
def subtract_dif(pkl_dict1, dif):
    ...
    for idx, preds1 in enumerate(pkl_dict1['preds']):
        ...
        pkl_dict1['preds'][idx] = np.where(pkl_dict1['preds'][idx] < 0, 0, pkl_dict1['preds'][idx])
...
if __name__ == "__main__":
    ...
    scores_dict = mult_perf_diagrams.verification(all_preds, all_labels, outdir)
